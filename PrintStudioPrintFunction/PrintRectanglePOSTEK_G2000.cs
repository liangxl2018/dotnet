...
namespace PrintStudioPrintFunction
{
    ...
    /// 打印矩形
    /// </summary>
    public class PrintRectanglePOSTEK_G2000 : IPrintFunction
    {
            ...
            try
            {
                PrintRuleBase.PTK_DrawRectangle
                    (
                        ...
                        (PrintRuleBase.GetPrintParameterByName<int>(printItem, "thickness", this.GetType().Name))*2/3,
                        (PrintRuleBase.GetPrintParameterByName<int>(printItem, "pEX", this.GetType().Name) + printItem.XDeviation)*2/3,
                        (PrintRuleBase.GetPrintParameterByName<int>(printItem, "pEY", this.GetType().Name) + printItem.YDeviation)*2/3
                    );
            }
            catch (Exception ex)
            {
